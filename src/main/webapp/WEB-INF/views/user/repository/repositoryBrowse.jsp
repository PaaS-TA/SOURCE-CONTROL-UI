...
<div class="rSearch_group">
    <div class="sel_group">
        ...
        <div class="selectbox select3 fr" style="width:300px;" id="fileSelect3">
            ...
            <ul class="select-list">
                <input type="text" name="copyUrl" id="copyUrl" maxlength="25" value="" title="레파지토리 클론" style="width:75%;">
                <button type="button" class="button sel_in_btn_lg" onclick='copycloneUrl(); return false;' maxlength="25" style="width:15%;">복사</button>
            </ul>
        </div>
...
<!--//셀렉트(브랜치, Tag) : e -->
<!-- list :s -->
<table id="broswerTable" summary="폴더/파일명, 최종 변경사항, 최종 변경자, 마지막업데이트 등을 나타낸 Commint 게시판입니다." class="basic_tbl w3-hoverable" >
    <caption>
        공지사항
    </caption>
    <colgroup>
        ...
        if (list.files == null || list.files.length == 0) {
            ...
            for (var i = 0; i < browser.length; i++) {
                ...
                if(browser[i].directory){
                    ...
                    sinnerHTML = sinnerHTML+'<td class="alignL" colspan="4"> <span class="ico_folder"></span>';
                    sinnerHTML = sinnerHTML+ browser[i].name +'</td></tr>'
                }else{
                    sinnerHTML = sinnerHTML+'<tr>';
                    sinnerHTML = sinnerHTML+'<td class="alignL"> <span class="ico_file"></span>';
                    sinnerHTML = sinnerHTML+ browser[i].name +'</td>'
